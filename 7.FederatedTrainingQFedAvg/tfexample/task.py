...
def load_data_crossdevice(Cid):
    ...
    #USAR LINEA ANTERIOR EN OTRAS IMPLEMENTACIONES, ESTOY AHORRANDO COMPUTACION
    ...
    # Cargar el archivo Excel, filtrar columnas
    data = pd.read_excel(os.path.join(ruta_archivos, archivo))
    data_cleaned = data[['Bolus', 'Basal', 'CGM(mg/dl)', 'Carb Input']].dropna()
    ...
    # Imprimir los primeros datos de y_test para debugging
...
def load_data_crosssilo(cid):
    ...
    # Cargar y concatenar todos los archivos en un único DataFrame, filtrar columnas necesarias
    dataframes = []
    for archivo in archivos_excel:
        print(f"Cargando el archivo: {archivo}")
        data = pd.read_excel(os.path.join(ruta_archivos, archivo))
