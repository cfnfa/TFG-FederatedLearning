...
def weighted_average(metrics):
    ...
    # Calcular las métricas ponderadas por el número de ejemplos
...
def server_fn(context: Context):
    ...
    # Define the strategy (FedTrimmedAvg o FedMedian)
    strategy =CustomFedAvg(
        run_config=context.run_config,
        use_wandb=context.run_config["use-wandb"],
        fraction_fit=context.run_config["fraction-fit"], # Sample 100% of available clients for training
        fraction_evaluate=0.8,  # 0,8 para cross-silo, 1 para cross-device
        min_fit_clients=8,  # 8 para cross-silo
